<script lang="ts">
	...
	function saveHistory() {
		localStorage.setItem('localama_chatHistory', JSON.stringify(history));
	}
...
<div class="flex min-h-screen flex-col bg-gray-50 dark:bg-slate-800">
	<main class="mx-auto mt-20 w-6xl flex-1 space-y-4 p-4">
			...
			{#each history as message}
				{#if message.role === 'user'}
					<div class="my-10 flex justify-end">
						<div
							class="max-w-lg rounded-lg border bg-gray-100 px-4 py-2.5 shadow dark:bg-slate-900"
						>
							{message.text}
						</div>
					</div>
				{:else}
					<div class="flex justify-start">
						<div class="mb-10 w-fit overflow-x-auto rounded-lg border px-4 py-2.5 shadow">
		...
		<ChatInput bind:prompt bind:isStreaming {startStream} {stopStream} />
	</main>
</div>
